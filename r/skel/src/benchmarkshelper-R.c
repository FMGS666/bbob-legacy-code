/*
 * R specific version of the benchmarkshelper.c file.
 *
 * NOTE: This file is a very ugly hack and should disappear. R does
 *   not like it if compiled code directly writes to the console or
 *   exits without giving R a change to clean up. We therefore need to
 *   replace the WARNING() and ERROR() functions. To better integrate
 *   with the rest of the R wrapper, these two functions have been
 *   moved to 'r_fgeneric.c'.
 */
...
void unif(double* r, int N, int inseed)
{
    ...
    for (i = 0; i < N; i++)
    {
        ...
        if (r[i] == 0.)
        {
            /* warning("Warning: zero sampled(?), set to 1e-99.\n"); */
            r[i] = 1e-99;
        }
    }
    return;
