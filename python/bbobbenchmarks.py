...
def compute_xopt(rseed, dim, show_u = False):
...
def fGauss(ftrue, beta):
    ...
    noise_value = np.exp(beta * _myrandn(popsi)) # with gauss noise
    fval = ftrue * noise_value
    tol = 1e-8
    fval = fval + 1.01 * tol
    idx = ftrue < tol
    try:
        fval[idx] = ftrue[idx]
    except: # fval is a scalar
        if idx:
            fval = ftrue
    return fval, noise_value

def fUniform(ftrue, alpha, beta):
    """Returns uniform model noisy value."""
    # expects ftrue to be a np.array
    popsi = np.shape(ftrue)
    noise_value = _myrand(popsi) ** beta * np.maximum(1., (1e9 / (ftrue + 1e-99)) ** (alpha * _myrand(popsi)))
    fval = ftrue * noise_value
    tol = 1e-8
    fval = fval + 1.01 * tol
    idx = ftrue < tol
...
def fCauchy(ftrue, alpha, p):
    ...
    # expects ftrue to be a np.array
    popsi = np.shape(ftrue)
    noise_value = alpha * np.maximum(0., 1e3 + (_myrand(popsi) < p) *
                                          _myrandn(popsi) / (np.abs(_myrandn(popsi)) + 1e-199))
    fval = ftrue + noise_value
    tol = 1e-8
    fval = fval + 1.01 * tol
    idx = ftrue < tol
...
class BBOBGaussFunction(BBOBFunction):
    ...
    def noise(self, ftrue):
        ...
        fvalue, noise_value = fGauss(ftrue, self.gaussbeta)
...
class BBOBUniformFunction(BBOBFunction, object):
    ...
    def noise(self, ftrue):
        """Returns the noisy function values."""
        fvalue, noise_value = fUniform(ftrue, self.unifalphafac * (0.49 + 1. / self.dim), self.unifbeta)
        self.noise_value = noise_value
        return fvalue
...
class BBOBCauchyFunction(BBOBFunction):
    ...
    def noise(self, ftrue):
        """Returns the noisy function values."""
        fvalue, noise_value = fCauchy(ftrue, self.cauchyalpha, self.cauchyp)
        self.noise_value = noise_value
        return fvalue
...
class _FRosenbrock(BBOBFunction):
    ...
    def initwithsize(self, curshape, dim):
        ...
        if self.dim != dim:
            ...
            #self.xopt = self.xopt * self.scales
